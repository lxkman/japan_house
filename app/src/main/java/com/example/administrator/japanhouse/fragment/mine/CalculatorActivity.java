...
import com.example.administrator.japanhouse.model.RichTextBean;
import com.example.administrator.japanhouse.presenter.RichTextPresenter;
...
public class CalculatorActivity extends AppCompatActivity implements View.OnClickListener, RichTextPresenter.RichTextCallBack {
...
//        initView();
        ...
        tv = (TextView) findViewById(R.id.fangdai_tv);

        initViewSpannable();
    }

    private void initViewSpannable() {
        RichTextPresenter presenter = new RichTextPresenter(this, this);

        presenter.getRichText(3);
    }
...
//    }
    ...
    @Override
    public void getRichText(Response<RichTextBean> response) {
        if (response != null && response.body() != null && response.body().getDatas() != null) {
            tv.setText(MyApplication.isJapanese() ? Html.fromHtml(response.body().getDatas().getContent_jpn()) : Html.fromHtml(response.body().getDatas().getContent_cn()));
        }
    }
}
