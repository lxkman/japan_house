                    ...
                    public void onSuccess(Response<OldHouseListBean> response) {
                        ...
                        if (mDatas == null || mDatas.size() == 0) {
                            if (datas == null || datas.size() == 0) {
                                Toast.makeText(ZhaoxiaoquActivity.this, "无数据~", Toast.LENGTH_SHORT).show();
                            ...
                            if (datas == null || datas.size() == 0) {
                                Toast.makeText(ZhaoxiaoquActivity.this, "没有更多数据了~", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            if (!isLoadMore) {
                                mDatas = datas;
                                Toast.makeText(ZhaoxiaoquActivity.this, "刷新成功~", Toast.LENGTH_SHORT).show();
                            } else {
                                mDatas.addAll(datas);
                            }
                            liebiaoAdapter.notifyDataSetChanged();
